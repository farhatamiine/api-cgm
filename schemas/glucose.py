...
class GlucoVariabilityResponse(BaseModel):
    ...
    cv:float = Field(...,description='Coefficient of Variation: The most important variability number (ideal is < 36%).')
    ...
    flag:str = Field(...,description='Glucose variability status (STABLE or HIGH VARIABILITY)')
...
class GlucosePatternResponse(BaseModel):
    ...
    worst_period: str = Field(..., description="The time of day with the highest average glucose")
...
class GlucoseDawnPhenomenon(BaseModel):
    ...
    flag: str = Field(..., description="Severity of dawn phenomenon (NONE, MILD, MODERATE, SEVERE)")
    interpretation: str = Field(..., description="Clinical interpretation of the dawn phenomenon data")
